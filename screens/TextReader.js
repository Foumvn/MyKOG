...
const TextReader = ({ route }) => {
  const { text } = route.params; // Récupérer les données audio passées

  return (
    <ScrollView style={styles.container}>
      <Image source={text.profileImage} style={styles.profileImage} />
      <Text style={styles.title}>{text.title}</Text>
      <Text style={styles.details}>{text.saison} | {text.date}</Text>
